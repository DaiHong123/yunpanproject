...
$('#searcher').on('click',function(){
	...
	$.ajax({
		...
		success:function(data){
	    	$('.filesListRoute').find('span').remove();
	    	$('.filesListRoute').append("<span><a onclick='asideAll()' style='cursor:pointer'>全部文件</a><span><span>>搜索：\""+ss+"\"</span>");
	    	document.getElementById('allChecks').checked = false;
	    	$("#filesTab").find("tr").remove();
	    	$(".filesListCount").find("span").remove();
	    	$(".filesListCount").append("<span>已加载</span><span class='filesCount'>"+data.length+"</span><span>个</span>");
...
$('#fileSearch').ajaxForm({
    ...
    success : function(data){
    	var names = $('.txt').val();
    	$('.filesListRoute').find('span').remove();
    	$('.filesListRoute').append("<span><a onclick='asideAll()' style='cursor:pointer'>全部文件</a><span><span>>搜索：\""+names+"\"</span>");
    	document.getElementById('allChecks').checked = false;
    	$("#filesTab").find("tr").remove();
    	$(".filesListCount").find("span").remove();
    	$(".filesListCount").append("<span>已加载</span><span class='filesCount'>"+data.length+"</span><span>个</span>");
